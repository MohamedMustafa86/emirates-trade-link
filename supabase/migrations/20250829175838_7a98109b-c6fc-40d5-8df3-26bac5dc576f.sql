...
CREATE TABLE public.unavailable_products (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE INDEX idx_search_queries_query_text ON public.search_queries USING gin(to_tsvector('arabic', query_text));

CREATE INDEX idx_unavailable_products_category ON public.unavailable_products(category_id);
CREATE INDEX idx_unavailable_products_status ON public.unavailable_products(status);
CREATE INDEX idx_unavailable_products_created_at ON public.unavailable_products(created_at);
...
-- Ensure all existing storage buckets have proper policies
-- Update product-images bucket policies
...
-- Update product-videos bucket policies
INSERT INTO storage.buckets (id, name, public) 
...
CREATE POLICY "Suppliers can upload product videos" 
ON storage.objects 
FOR INSERT 
WITH CHECK (
...
-- Update user-avatars bucket policies
INSERT INTO storage.buckets (id, name, public) 
...
-- Update supplier-logos bucket policies
INSERT INTO storage.buckets (id, name, public) 
